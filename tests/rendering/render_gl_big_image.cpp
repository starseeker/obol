/*
 * render_gl_big_image.cpp - Integration test using shared testlib factory.
 *
 * Uses ObolTest::Scenes::createGLBigImage — the same factory used by the
...
int main(int argc, char **argv)
{
    ...
    SoSeparator *root = ObolTest::Scenes::createGLBigImage(W, H);
    ...
    printf("render_gl_big_image: ok=%d nonbg=%d\n", ok, nb);
    renderer.writeToRGB(outpath);

    root->unref();

    if (!ok || nb < 100) {
        fprintf(stderr, "render_gl_big_image: FAIL – scene blank (nb=%d)\n", nb);
        return 1;
    }
    printf("render_gl_big_image: PASS\n");
    return 0;
}
